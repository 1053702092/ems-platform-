#define S_FUNCTION_NAME  s_ecms_opti
#define S_FUNCTION_LEVEL 2

#include "simstruc.h"
#include "casadi_functions.h"
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    int_T n_in  = ecms_opti_n_in();
    int_T n_out = ecms_opti_n_out();
    int_T sz_arg, sz_res, sz_iw, sz_w;
    ecms_opti_work(&sz_arg, &sz_res, &sz_iw, &sz_w);
    
    /* Set up simulink input/output ports */
    int_T i;
    if (!ssSetNumInputPorts(S, n_in)) return;
    for (i=0;i<n_in;++i) {
      const int_T* sp = ecms_opti_sparsity_in(i);
    ...
    for (i=0;i<n_out;++i) {
      const int_T* sp = ecms_opti_sparsity_out(i);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    */
    ...
    int mem = ecms_opti_checkout();

    /* Run the CasADi function */
    ecms_opti(arg,res,iw,w,mem);

    /* Release hold */
    ecms_opti_release(mem);
}
